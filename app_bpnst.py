...
try:
    ...
    st.error("Spreadsheet ID belum dikonfigurasi di Streamlit Secrets. Menggunakan placeholder untuk demonstrasi.")
    SPREADSHEET_ID = "PLACEHOLDER_ID"

# Memuat data (Ganti dengan data riil atau mock jika offline)
try:
    df_wilayah = load_data(SPREADSHEET_ID, "1848496896")
    df_pegawai = load_data(SPREADSHEET_ID, "1168898330")
except Exception as e:
    st.warning("Gagal memuat data otomatis dari Google Sheets. Membuat struktur DataFrame kosong untuk inisialisasi.")
    # Struktur Data GID 1848496896
    df_wilayah = pd.DataFrame(columns=[
        'kabupaten_kota', 'kecamatan', 'desa_kelurahan', 'kode_wilayah', 'luas_adm', 
        'luas_apl', 'jumlah_persil', 'luas_persil', 'luas_persil_valid', 'jumlah_kw456', 
        'luas_kw456', 'jumlah_bt', 'bt_valid', 'luas_persil_deliniasi', 'pra_sertel', 
        'pra_btel', 'jumlah_su', 'jumlah_suvalid', 'pra_suel'
    ])
    # Struktur Data GID 1168898330
    df_pegawai = pd.DataFrame(columns=[
        'satker', 'nip', 'nama', 'jabatan', 'url', 'jenis_jabatan', 'kategori_asn', 'target_dipa', 'realisasi_dipa'
    ])

# Membersihkan data teks dari spasi berlebih
...
for col in ['satker', 'nama', 'jabatan', 'kategori_asn']:
...
# Filter Kabupaten/Kota (Ditambahkan opsi "Sulawesi Tengah")
...
st.sidebar.markdown("---")
st.sidebar.subheader("Profil Pejabat Struktural")

# Menyaring data pegawai berdasarkan Satker yang relevan dengan filter Kabupaten
# Asumsi nama Satker mengandung nama Kabupaten atau jika Sulawesi Tengah mengambil Kantor Wilayah (Kanwil)
df_peg_filtered = df_pegawai.copy()
if selected_kab != "Sulawesi Tengah":
    # Cari satker yang memuat nama kabupaten secara case-insensitive
    df_peg_filtered = df_pegawai[df_pegawai['satker'].str.contains(selected_kab, case=False, na=False)]
else:
    # Jika Sulawesi Tengah, tampilkan tingkat Provinsi / Kanwil
    df_peg_filtered = df_pegawai[df_pegawai['satker'].str.contains("Kanwil", case=False, na=False)]

# Fungsi bantu untuk merender profil pegawai di sidebar sesuai urutan jabatan
def render_profile(jabatan_keyword):
    # Cari yang jabatannya mengandung kata kunci tertentu
    ...
    if not row.empty:
        ...
        # Menggunakan placeholder gambar jika URL kosong atau tidak valid
        ...
        # Progress Bar Representatif Persentase Realisasi DIPA
        st.sidebar.progress(min(float(pct/100), 1.0))
        st.sidebar.caption(f"Realisasi DIPA: {pct:.2f}% (Rp {row['realisasi_dipa']:,.0f})")
    else:
        st.sidebar.caption(f"Data jabatan mengandung '{jabatan_keyword}' tidak ditemukan")
...
order_jabatan = [
    ...
    "Penata dan Pemberdayaan", # Mengakomodasi variasi teks Penataan dan Pemberdayaan
...
# ==========================================
st.title(f"📊 Dashboard Kinerja & Agraria - Wilayah {selected_kab}")
if selected_kec != "Semua Kecamatan":
    st.subheader(f"Kecamatan: {selected_kec}")

# Menyiapkan Filter Data Utama untuk Main Content
...
# Baris Top Header: URL 2, URL 1, Pie Chart, dan Metrik Cards
...
with row_top[1]:
    ...
    df_kakan = df_peg_filtered[df_peg_filtered['jabatan'].str.contains("Kepala Kantor|Kakan", case=False, na=False)]
...
with row_top[2]:
    # Grafik Pie Total Realisasi DIPA vs Sisa Target DIPA
...
with row_top[3]:
    # Grid 5 Kolom untuk Card Metrik internal
    c_sdm, c_apl, c_kec, c_desa, c_kw = st.columns(5)
    
    with c_sdm:
        # Card SDM berdasarkan jumlah nama unik di satker terpilih
        jumlah_sdm = df_peg_filtered['nama'].nunique()
        # breakdown berdasarkan kategori_asn
        ...
        <div class="custom-card">
            <div class="card-title">Jumlah SDM</div>
            <div class="card-value">{jumlah_sdm}</div>
            <div class="card-subtext">{sub_asn_text}</div>
        </div>
        ''', unsafe_allow_html=True)
        
    with c_apl:
        # Card Luas APL & % Luas APL / Luas ADM
        ...
        st.markdown(f'''
        <div class="custom-card">
            <div class="card-title">Luas APL</div>
            <div class="card-value">{tot_apl:,.1f} Ha</div>
            <div class="card-subtext">{pct_apl:.2f}% dari Luas ADM</div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with c_kec:
        ...
        if selected_kab == "Sulawesi Tengah":
            # Jika Sulawesi Tengah, tampilkan jumlah kabupaten/kota lainnya
        ...
        st.markdown(f'''
        <div class="custom-card">
            <div class="card-title">Kecamatan</div>
            <div class="card-value">{val_kec}</div>
            <div class="card-subtext">{lbl_kec}</div>
        </div>
        ''', unsafe_allow_html=True)
        
    with c_desa:
        # Card Jumlah Desa/Kelurahan beserta breakdown jika ada kolom penanda tipe (jika tidak, hitung total)
        # Karena di index hanya ada desa_kelurahan, hitung total item unik atau berasumsi klasifikasi string nama
        if selected_kab == "Sulawesi Tengah":
            val_desa = df_wilayah['kabupaten_kota'].nunique() # Representasi entitas kab sesuai instruksi "tampil kab/kota lainnya"
            sub_desa_text = "Seluruh Wilayah Sulteng"
        else:
            val_desa = df_wil_filtered['desa_kelurahan'].nunique()
            # Simulasi deteksi Kelurahan vs Desa berdasarkan string awalan jika memungkinkan, atau tampilkan nilai total
            sub_desa_text = f"Total Desa & Kelurahan"
            
        st.markdown(f'''
        <div class="custom-card">
            <div class="card-title">Desa / Kelurahan</div>
            <div class="card-value">{val_desa}</div>
            <div class="card-subtext">{sub_desa_text}</div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with c_kw:
        ...
        st.markdown(f'''
        <div class="custom-card">
            <div class="card-title">Jumlah KW456</div>
            <div class="card-value">{val_kw}</div>
            <div class="card-subtext">{lbl_kw}</div>
        </div>
        ''', unsafe_allow_html=True)
...
# Agregasi data berdasarkan scope wilayah untuk keperluan chart bar axis-X
if selected_kab == "Sulawesi Tengah":
    # Kelompokkan per Kabupaten/Kota jika cakupan Provinsi
    df_chart = df_wilayah.groupby('kabupaten_kota').sum().reset_index()
    x_axis_column = 'kabupaten_kota'
elif selected_kec == "Semua Kecamatan":
    # Kelompokkan per Kecamatan jika cakupan Kabupaten
    df_chart = df_wil_filtered.groupby('kecamatan').sum().reset_index()
    x_axis_column = 'kecamatan'
else:
    # Kelompokkan per Desa/Kelurahan jika cakupan spesifik Kecamatan
...
if not df_chart.empty:
    ...
    st.info("Data Pemetaan & Validasi Persil tidak tersedia untuk filter ini.")
...
if not df_chart.empty:
    ...
    st.info("Data Validasi Buku Tanah tidak tersedia untuk filter ini.")
